...
for ts, its in zip( tsink, range( tsinkNum ) ) :
    ...
    for imom in range( momBoostNum ):
        ...
        threeps = rw.readAvgXFile_imag( threepDir, configList_loc, \
    ...
    mpi_fncs.mpiPrintAllRanks( threep_mom, comm )
    ...
    if rank == 0:
        ...
        pSq = (2.0*np.pi/L)**2 * momSq
        print(pSq)
        energy = np.sqrt( mEff_fit**2 + pSq )
        print(energy)
        preFactor = np.repeat( -2.0 / mEff_fit**2 * \
                               energy * ( energy + mEff_fit ) \
                               / ( 3.0 * energy**2 + pSq ), \
