...
for ts in tsink:
    ...
    #preFactor = -2.0 * Z / mEff_fit_cp**2 * energy_cp * ( energy_cp + mEff_fit_cp ) \
        #/ ( 3 * energy_cp**2 + pSq_cp )

    #avgX = preFactor * threep_jk[-1][:,:ts+1] / fit.twopExp( ts, G, E )

    avgX = Z * pq.calcAvgX_momBoost( threep_jk[ -1 ], twop_jk[ :, ts ], mEff_fit, momSq, L )
    ...
    avgX_outFilename = output_template.replace( "*", "avgX_u_tsink" + str( ts ) + "_momSq" + str( momSq ) )
    ...
    if particle == "kaon":
        ...
        avgX_s_outFilename = output_template.replace( "*", "avgX_s_tsink" + str( ts ) + "_momSq" + str( momSq ) )
